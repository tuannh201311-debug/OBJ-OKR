...
def generate_team_weekly_report(week: int, year: int) -> bytes:
    ...
    for report in reports:
        ...
        pdf.cell(0, 12, f"  Nhân sự: {user_name}", ln=True, fill=True)
        pdf.ln(2)
        ...
        if not done_tasks:
            pdf.cell(0, 6, "  - Không có", ln=True)
        else:
            for t in done_tasks:
                progress = t.get("progress", 0)
                bt_title = t.get("big_task_title", "Không có")
                pdf.multi_cell(0, 6, f"  - [{progress}%] {t.get('title', '')} (OBJ: {t.get('okr_title', '')} | Plan: {bt_title})")
        pdf.ln(2)
        ...
        pdf.set_font("Roboto", "", 10)
        pdf.set_text_color(15, 23, 42)
        pdf.multi_cell(0, 6, challenges if challenges else "  - Không có")
        pdf.ln(2)
        ...
        pdf.set_font("Roboto", "", 10)
        pdf.multi_cell(0, 6, next_plan if next_plan else "  - Chưa lập kế hoạch")
